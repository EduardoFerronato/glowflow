...
export function TopProcedures({ procedures }: { procedures: ProcedureRow[] }) {
  const max = Math.max(...procedures.map((p) => p.count), 1)
      ...
      <CardContent>
        {procedures.length === 0 ? (
          <EmptyState
            ...
            description="O ranking do mês aparecerá aqui."
          />
        ) : (
              ...
              <li key={p.id} className="space-y-1.5">
                ...
                <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                  <div
                    ...
                    style={{
                      width: `${Math.max((p.count / max) * 100, 6)}%`,
